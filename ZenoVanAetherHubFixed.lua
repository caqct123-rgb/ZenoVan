...
local RemoveGUIRunning = false
local MissionToggle = false
...
local MissionList = {"Necromancer", "Commander", "Ranger", "Shotgunner"}
...
local function DisableGui() 
    if RemoveGUIRunning or not Globals.DisableGUI then 
        return 
    end
    RemoveGUIRunning = true 
    task.spawn(function()
        
        while true do
            local hiddenUI = CoreGui:FindFirstChild("HiddenUI")
            if hiddenUI then
                local lunaUI = hiddenUI:FindFirstChild("Luna UI")
                if lunaUI then
                    local property = lunaUI:IsA("ScreenGui") and "Enabled" or "Visible"
                    
                    if lunaUI[property] == true then
                        lunaUI[property] = false
                        print("Luna UI forced to false.")
                    end
                end
            end
...
local function checkMissionStatus()
    while Globals.MissionToggle do
        -- Safely attempt to get the description text
        local playerGui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
        local descriptionObj = playerGui 
            and playerGui:FindFirstChild("ReactUniversalObjectives")
            -- Navigating the specific frame path you provided
            and playerGui.ReactUniversalObjectives:FindFirstChild("Frame")
            and playerGui.ReactUniversalObjectives.Frame:FindFirstChild("1")
            and playerGui.ReactUniversalObjectives.Frame["1"]:FindFirstChild("task")
            and playerGui.ReactUniversalObjectives.Frame["1"].task:FindFirstChild("description")

        if descriptionObj then
            local currentText = descriptionObj.Text
            
            -- Check based on what is selected in the Dropdown
            if Globals.MissionValue == "Ranger" then
                if string.find(currentText, "Deal 250,000 damage with the Ranger") then
                    print("Ranger")
                end
            elseif Globals.MissionValue == "Necromancer" then
                if string.find(currentText, "Necromancer") then -- Adjust string match as needed later
                    print("Necromancer")
                end
            elseif Globals.MissionValue == "Commander" then
                if string.find(currentText, "Commander") then -- Adjust string match as needed later
                    print("Commander")
                end
            elseif Globals.MissionValue == "Shotgunner" then
                if string.find(currentText, "Deal 100,000 Damage with Shotgunner tower") then -- Adjust string match as needed later
                    print("Shotgunner")
                end
            end
        else
            warn("Mission description text object not found in PlayerGui yet.")
        end
        
        task.wait(1) -- Adjust the delay (in seconds) so it doesn't lag your game
    end
end
...
local Configuration = Window:Tab({Title = "Configuration", Icon = "sliders-horizontal"}) do
    ...
    Configuration:Section({Title = "Misc"})

        Configuration:Toggle({
        Title = "Remove Railgun GUI",
        Desc = "",
        Value = Globals.DisableGUI,
        Callback = function(v)
            SetSetting("DisableGUI", v)
            DisableGui()
        end
    })
...
-- 1. The trial mapping table
local NextTrialMap = {
    ["SpeedyEnemies"]   = "Glass",
    ["Glass"]           = "Quarantine",
    ["Quarantine"]      = "Fog",
    ["Fog"]             = "Limitation",
    ["Limitation"]      = "FlyingEnemy",
    ["FlyingEnemy"]     = "JailedTowers",
    ["JailedTowers"]    = "ExploadingEnemy",
    ["ExploadingEnemy"] = "Inflation",
    ["Inflation"]       = "Commited",
    ["Commited"]        = "HiddenEnemies",
    ["HiddenEnemies"]   = "Broke",
    ["Broke"]           = "HealthyEnemy",
    ["HealthyEnemy"]    = "SpeedyEnemies"
}

-- 2. A shared table to hold the live data strings
local TrialData = {
    Current = "Loading...",
    TimeLeft = "Loading...",
    Next = "Loading..."
...
-- 3. The loop function (completely independent of Strategies:Label)
task.spawn(function()
    while true do
        local currentText = workspace.ActiveTrial.SurfaceGui.Trial.Text
        local timeText = workspace.ActiveTrial.SurfaceGui.TrialEnds.TrialTimer.Text
        
        -- Update the shared table values
        TrialData.Current = currentText
        TrialData.TimeLeft = timeText
        TrialData.Next = NextTrialMap[currentText] or "Unknown"
...
local Strategies = Window:Tab({Title = "Strategies", Icon = "clipboard-list"}) do
    ...
    Strategies:Section({Title = "Auto Strategies"})

    Strategies:Dropdown({
    Title = "Select Map:",
    List = MapList,
    Value = Globals.MapValue or "None",
    Callback = function(choice)
        Globals.MapValue = choice
        SetSetting("MapValue", choice)
    end
    })
...
Strategies:Label({
    Title = "Trial Info", 
    Desc = "Current Trial: " .. TrialData.Current .. "SES\n" ..
           "Time left: " .. TrialData.TimeLeft .. "\n" ..
           "Next Trial: " .. TrialData.Next
})

    Strategies:Dropdown({
        Title = "Select Mission",
        Desc = "Choose a mission to target",
        List = MissionList,
        Value = Globals.MissionValue,
        Callback = function(choice)
            local selected = choice
            if type(choice) == "table" then selected = choice[1] end
            if not selected or selected == "" then selected = "None" end
            
            Globals.MissionValue = selected
            SetSetting("SelectedMission", selected)
            print("Strategy mission updated to: " .. tostring(Globals.MissionValue))
        end
    })


    -- NEW: Separate Toggle for Mission Strategy
  Strategies:Toggle({
        Title = "Auto Mission Mode",
        Desc = "Automatically starts the selected mission strategy",
        Value = Globals.MissionToggle,
        Callback = function(v)
            Globals.MissionToggle = v
            SetSetting("MissionToggle", v)

            if v then
                print("Mission mode activated for: " .. tostring(Globals.MissionValue))
                -- Spawns the local loop function on a separate thread so it doesn't freeze the UI
                task.spawn(checkMissionStatus)
            else
                print("Mission mode deactivated.")
            end
        end
    })
...
function TDS:RemoveIndex()
    ...
    -- 2. Clear out Strategy & Upgrade Level Trackers
